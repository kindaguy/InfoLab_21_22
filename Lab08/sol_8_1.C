...
int main(){
   ...
   for(int i=0; i<50; i++){
      rilevazioni[i] = rilev(N);
   }

   av = media(rilevazioni, M);
   var = pow(devstd(rilevazioni,M),2);

   cout << endl<< "Stima Monte-Carlo pigreco con" << endl;
   cout << M << " rilevazioni di" << endl;
   cout << N << " punti ciascuna" << endl;
   cout << "media rilevazioni: " << av << endl;
   cout << "varianza rilevazioni: " << var << endl;
